...
Thay `metus.com.vn` bằng domain thật ở các bước bên dưới.
...
Mỗi thôn dùng 1 subdomain (`slug.metus.com.vn`) trỏ về cùng 1 tiến trình Next.js — middleware trong FE tự tách slug từ subdomain, không cần cấu hình riêng cho từng thôn ở Nginx.

### DNS — tại nhà cung cấp domain

| Loại | Giá trị |
|---|---|
| A record | `@ → <IP VPS>` (cho `metus.com.vn`) |
| A record (wildcard) | `* → <IP VPS>` (cho `*.metus.com.vn`) |
...
server {
    listen 80;
    server_name metus.com.vn *.metus.com.vn;
...
Chứng chỉ wildcard (`*.metus.com.vn`) bắt buộc phải xác thực qua **DNS-01** — thử thách HTTP-01 thông thường không cấp được cho wildcard.
...
server {
    ...
    ssl_certificate     /etc/letsencrypt/live/metus.com.vn/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/metus.com.vn/privkey.pem;
...
1. Vào `https://metus.com.vn/superadmin`, đăng nhập tài khoản superadmin.
...
4. Thôn mới lập tức truy cập được ở `https://<slug>.metus.com.vn` — không cần restart gì trên VPS.
